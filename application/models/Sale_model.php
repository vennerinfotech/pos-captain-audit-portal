...
class Sale_model extends CI_Model {
    ...
    public function getSaleList($outlet_id) {
        $result = $this->db->query("SELECT s.*,u.full_name,m.name
          FROM tbl_sales s
          ...
          /*$result = $this->db->query("SELECT * FROM tbl_sales s WHERE s.order_status = '3' AND s.del_status = 'Live' AND s.outlet_id=$outlet_id ORDER BY s.id DESC")->result();*/
        return $result;
    }
    /**
    ...
    public function getWaitersForThisCompany($company_id,$table){
        ...
        if(str_rot13($language_manifesto)=="eriutoeri"){
            ...
            $this->db->from($table);
            $this->db->where("company_id", $company_id);
            $this->db->where("designation", 'Champion');
            $this->db->where("del_status", 'Live');
    ...
    public function getSaleBySaleId($sales_id){
      $this->db->select("tbl_sales.*,w.full_name as waiter_name,tbl_customers.name as customer_name,tbl_customers.address as customer_address,tbl_customers.phone as customer_phone,tbl_tables.name as table_name,tbl_users.full_name as user_name,tbl_companies.invoice_footer as invoice_footer");
      $this->db->from('tbl_sales');
      $this->db->join('tbl_customers', 'tbl_customers.id = tbl_sales.customer_id', 'left');
      $this->db->join('tbl_users', 'tbl_users.id = tbl_sales.user_id', 'left');
      $this->db->join('tbl_tables', 'tbl_tables.id = tbl_sales.table_id', 'left');
    ...
    /**
     * get total kitchen type started Preparing items
     * @access public
     * @return object
     * @param int
     */
    public function get_total_kitchen_type_started_cooking_items($sale_id)
    {
        ...
        $this->db->from('tbl_sales_details');
        $this->db->where("sales_id", $sale_id);
        $this->db->where("cooking_status", "Started Preparing");
        return $this->db->get()->num_rows();
    }
    /**
