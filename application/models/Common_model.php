...
class Common_model extends CI_Model {
     ...
     * @param string
     */
    public function getAllstaffByUserIdForDropdown($user_id, $table_name) {
        $result = $this->db->query("SELECT * 
          FROM $table_name 
          WHERE user_id=$user_id AND del_status = 'Live'  
          ORDER BY 2")->result();
        return $result;
    }
     ...
     * @param string
     */
    public function getIngredients($company_id, $table_name) {
        $result = $this->db->query("SELECT *, (SELECT unit_name from tbl_units where id = $table_name.unit_id) as unitname
          FROM $table_name 
          WHERE company_id=$company_id AND del_status = 'Live'  
          ORDER BY name asc")->result();
        return $result;
     ...
     * @param string
     */
    public function getClosings($company_id, $table_name) {
        $result = $this->db->query("SELECT *, (SELECT unit_name from tbl_units where id = $table_name.unit_id) as unitname
          FROM $table_name 
     ...
     * @param string
     */
    public function getIngredientNameById($company_id, $table_name) {
     ...
     * @param string
     */
    public function getAllUserByOutletForDropdown($company_id, $table_name) {
        $outlet_id = $this->session->userdata('outlet_id');
        if($outlet_id == 1){
           $result = $this->db->query("SELECT * 
              FROM $table_name 
              WHERE company_id=$company_id AND del_status = 'Live'  
              ORDER BY company_id")->result();
            return $result; 
        }
        else
        {
