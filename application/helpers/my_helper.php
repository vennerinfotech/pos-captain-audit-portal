...
function staffName($id) {
    $CI = & get_instance();
    $employee_information = $CI->db->query("SELECT * FROM tbl_outet_staff where `id`='$id'")->row();
    if (!empty($employee_information)) {
        return $employee_information->staff_name;
    }else{
        return "N/A";
    }
}
 ...
 * @param int
 */
function getOutletAddressById($outlet_id){
    $CI = & get_instance();
    $outlet_info1 = $CI->db->query("SELECT * FROM tbl_outlets WHERE id='$outlet_id' AND del_status='Live'")->row();
    if($outlet_info1){
        return $outlet_info1->address;
    }else{
        return "";
    }
}
...
function getOrderType($order_type_id) {
    if ($order_type_id == 1) {
        ...
        return "Swiggy";
    }elseif ($order_type_id == 5) {
        return "Zomato";
    }elseif ($order_type_id == 6) {
        return "Channel 1";
    }elseif ($order_type_id == 7) {
        return "Channel 2";
    }elseif ($order_type_id == 8) {
        return "Channel 3";
    }
}
/**
