<?php 
 ...
 }elseif($sale_object->order_type=='4'){
    $order_type = 'Swiggy';
    $order_number = 'D '.$sale_object->sale_no;    
 }elseif($sale_object->order_type=='5'){
    $order_type = 'Zomato';
    $order_number = 'E '.$sale_object->sale_no;    
 }elseif($sale_object->order_type=='6'){
    $order_type = 'Channel 1';
    $order_number = 'F '.$sale_object->sale_no;    
 }elseif($sale_object->order_type=='7'){
    $order_type = 'Channel 2';
    $order_number = 'G '.$sale_object->sale_no;    
 }elseif($sale_object->order_type=='8'){
    $order_type = 'Channel 3';
    $order_number = 'H '.$sale_object->sale_no;    
 }
...
<body>
    <div id="wrapper">
        <div id="receiptData">
            ...
            <div id="receipt-data">
                <div class="ir_txt_center" style="font-size: 50px;">
                    <h3 class="ir_txt_center"><?php echo lang('KOT'); ?></h3>
                  <?php echo lang('Invoice_No'); ?><b>: </b><?= escape_output($order_number) ?><br>
                   <?php echo lang('date'); ?>: <?= escape_output(date($this->session->userdata('date_format'), strtotime($sale_object->sale_date))); ?>
                ...
                <?php echo lang('customer'); ?><b>: </b>
                   ...
                   <?php echo lang('order_type'); ?>:
                    <?php echo escape_output($order_type); ?> <?= escape_output($sale_object->waiter_name ? lang('waiter').": " . $sale_object->waiter_name : '') ?>
                    <br>
                </div>
                ...
                <table class="table table-condensed">
                    <thead>
                        <tr class="ir_font_bold">
                            <td class="ir_w_5" style="font-size: 15px;"><?php echo lang('sn'); ?></td>
                            <td class="ir_w_85" style="font-size: 15px;"><?php echo lang('item'); ?></td>
                            <td class="ir_w_10 ir_txt_center" style="font-size: 11px;"><?php echo lang('qty'); ?></td>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            if (isset($sale_object->items)) {
                                ...
                                foreach ($sale_object->items as $row) {
                                    ...
                                    ?>
                        <tr>
                            <td style="font-size: 11px;"><?php echo escape_output($i++); ?></td>
                            <td style="font-size: 11px;">
                                ...
                                <?php if($row->menu_note!=""){ echo lang('note').":".$row->menu_note; }?>
                            </td>
                            <td class="ir_txt_center" style="font-size: 11px;"><?php echo escape_output($row->tmp_qty) ?> </td>
                        </tr>
                        <?php endif; }} ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <th class="ir_txt_center" colspan="6" style="font-size: 15px;"><?php echo lang('Total_Item_s'); ?>: <?php echo escape_output($totalItems); ?></th>
                        </tr>
                    </tfoot>
                </table>
