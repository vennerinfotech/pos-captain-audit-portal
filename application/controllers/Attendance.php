...
class Attendance extends Cl_Controller {
    ...
    public function attendances() { 
        ...
        if($user_id == 1){
            ...
            $data['attendances'] = $this->db->query("select * from tbl_attendance where user_id = $user_id and company_id=$company_id and del_status='Live' order by id desc")->result();
        } 
    ...
    public function addEditAttendance($encrypted_id='') { 
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == TRUE) {
				...
				if ($encrypted_id != '') {
                    $information['out_img'] =htmlspecialchars($this->input->post($this->security->xss_clean('dataurl')));
                }else{
                    $information['in_img'] =htmlspecialchars($this->input->post($this->security->xss_clean('dataurl')));
                }
            ...
            if ($id=='') { 
				...
				$user_id = $this->session->userdata('user_id');
                $company_id = $this->session->userdata('company_id');
                $data['employees'] = $this->Common_model->getAllstaffByUserIdForDropdown($user_id, "tbl_outet_staff");
                $data['main_content'] = $this->load->view('attendance/addEditAttendance', $data, TRUE); 
                $this->load->view('userHome', $data);
            }else{
                $data = array();
