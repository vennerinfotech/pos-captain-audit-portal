...
class Report extends Cl_Controller {
    ...
    public function saleReportByMonth() {
        ...
        if ($this->input->post('submit')) {
            ...
            $outlet_id =htmlspecialchars($this->input->post($this->security->xss_clean('outlet_id')));
            $data['user_id'] = $user_id;
            if ($start_date && $end_date) {
            ...
            $data['saleReportByMonth'] = $this->Report_model->saleReportByMonth($start_date, $end_date, $user_id, $outlet_id);
        }


        $data['users'] = $this->Common_model->getAllUserByOutletForDropdown($company_id, 'tbl_users');
