...
class Authentication extends Cl_Controller {
    ...
    public function userProfile() {
        ...
        if($this->session->userdata('role') == 'Cafe User'){
            redirect("Bar/panel");
        }
        if($this->session->userdata('role') == 'Champion User'){
            redirect("Waiter/panel");
        }
        if($this->session->userdata('role') == 'POS User'){
            redirect("Sale/POS");
        }
        if($this->session->userdata('role') == 'Orderstatus User'){
            redirect("Orderstatus/panel");
        }
        ...
        if($sale_id){
            ...
            if(isset($company->receipt_printer_invoice) && $company->receipt_printer_invoice){
                ...
                if($sale->order_type == 1){
                    ...
                    $order_type = 'Swiggy';
                }elseif($sale->order_type == 5){
                    $order_type = 'Zomato';
                }elseif($sale->order_type == 6){
                    $order_type = 'Channel 1';
                }elseif($sale->order_type == 7){
                    $order_type = 'Channel 2';
                }elseif($sale->order_type == 8){
