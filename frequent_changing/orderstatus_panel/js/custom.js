...
$(document).ready(function () {
  ...
  $(document).on("click", ".items_holder .single_item", function () {
    ...
    if (single_order.attr("data-order-type") == "Dine In") {
      if ($(this).attr("data-selected") == "selected") {
        ...
        $(this).find(".single_item_cooking_status").css("color", "");
        if (
  ...
  $(document).on("click", ".select_all_of_an_order", function () {
    ...
    $("#single_order_" + order_id + " .items_holder .single_item")
      ...
      .css("color", "#fff");
    $("#single_order_" + order_id + " .items_holder .single_item")
      .find(".single_item_cooking_status")
      .css("color", "#fff");
    $("#single_order_" + order_id + " .items_holder .single_item").attr(
      "data-selected",
      "selected"
  ...
  $(document).on("click", ".start_cooking_button", function () {
    ...
    if ($("#single_order_" + sale_id).attr("data-order-type") == "Dine In") {
      if (
        ...
        if (previous_id != "") {
          let url = base_url + "Orderstatus/update_cooking_status_ajax";
          $.ajax({
            url: url,
            method: "POST",
            data: {
      ...
      if (previous_id != "") {
        let url =
          base_url + "Orderstatus/update_cooking_status_delivery_take_away_ajax";
        $.ajax({
          url: url,
          method: "POST",
          data: {
  ...
  /*$(document).on("click", "#start_cooking", function () {
    if (
      ...
      if (
        ...
        if (previous_id != "") {
          $.ajax({
            url: base_url + "Orderstatus/update_cooking_status_ajax",
            method: "POST",
            data: {
              previous_id: previous_id,
              cooking_status: "Started Preparing",
  ...
  $(document).on("click", "#order_details_holder .single_order", function () {
    ...
    $.ajax({
      url: base_url + "Orderstatus/get_order_details_orderstatus_ajax",
      method: "POST",
      data: {
        sale_id: sale_id,
        csrf_irestoraplus: csrf_value_,
  ...
  $(document).on("click", "#notification_remove_all", function () {
    if ($(".single_notification_checkbox:checked").length > 0) {
      ...
      if (notifications != "") {
        ...
        $.ajax({
          url: base_url + "Orderstatus/remove_multiple_notification_ajax",
          method: "POST",
          data: {
  ...
  $(document).on("click", ".single_serve_b", function () {
    ...
    $.ajax({
      url: base_url + "Orderstatus/remove_notication_ajax",
      method: "POST",
      data: {
...
function new_notification_interval() {
  $.ajax({
    url: base_url + "Orderstatus/get_new_notifications_ajax",
    method: "POST",
    data: {
      csrf_irestoraplus: csrf_value_,
    },
...
let bell_new_order = new Howl({
  src: [base_url + "assets/media/orderstatus_bell.mp3"],
});

refresh_orders();
function refresh_orders() {
  let url = base_url + "Orderstatus/get_new_orders_ajax";
  $("#refresh_it_or_not").html("Yes");
  $.ajax({
    url: url,
    method: "POST",
    ...
    success: function (response) {
      ...
      for (let key in response) {
        ...
        if (response[key].order_type == "1") {
          ...
          order_type = "Delivery";
        } else if (response[key].order_type == "4") {
          order_name = "D " + response[key].sale_no;
          order_type = "Swiggy";
        } else if (response[key].order_type == "5") {
          order_name = "E " + response[key].sale_no;
          order_type = "Zomato";
        } else if (response[key].order_type == "6") {
          order_name = "F " + response[key].sale_no;
          order_type = "Channel 1";
        } else if (response[key].order_type == "7") {
          order_name = "G " + response[key].sale_no;
          order_type = "Channel 2";
        } else if (response[key].order_type == "8") {
          order_name = "H " + response[key].sale_no;
          order_type = "Channel 3";
        }
        //for new order play bell
        if (Number(response[key].is_orderstatus_bell) == 1) {
          bell_new_order.play();
        }
        let tables_booked = "";
        ...
        let selected_background =
          ...
          response[key].sales_id
            ...
            : "";
        let width = 100;
        let total_orderstatus_type_items = response[key].total_orderstatus_type_items;
        let total_orderstatus_type_started_cooking_items =
          response[key].total_orderstatus_type_started_cooking_items;
        let total_orderstatus_type_done_items =
          response[key].total_orderstatus_type_done_items;
        let splitted_width = (
          parseFloat(width) / parseFloat(total_orderstatus_type_items)
        ).toFixed(2);
        let percentage_for_started_cooking = (
          parseFloat(splitted_width) *
          parseFloat(total_orderstatus_type_started_cooking_items)
        ).toFixed(2);
        let percentage_for_done_cooking = (
          parseFloat(splitted_width) * parseFloat(total_orderstatus_type_done_items)
        ).toFixed(2);
        ...
        if (total_orderstatus_type_items != total_orderstatus_type_done_items) {
          $order_list_left +=
          ...
          // $order_list_left += '<p class="order_number">Customer Name: '+customer_name+'</p> ';
          // $order_list_left += '<p class="order_number">Waiter Name: '+waiter_name+'</p> ';
          $order_list_left +=
            '<p class="order_number">Table: ' + tables_booked + "</p> ";
          // $order_list_left += '<p class="order_number">Order Type: '+order_type+'</p> ';
          $order_list_left += "</div>";
          $order_list_left += '<div class="fix floatleft" style="width:30%;">';
          $order_list_left +=
            '<p class="order_duration dark-blue-background"><span id="orderstatus_time_minute_' +
            response[key].sales_id +
            '">' +
            minute +
            '</span>:<span id="orderstatus_time_second_' +
            response[key].sales_id +
            '">' +
            second +
            "</span></p>";
